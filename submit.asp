...
' job_type[] is an array if multiple checkboxes are checked jobTypes=Request.Form("job_type") Dim jobStr jobStr="" If
    IsArray(jobTypes) Then For i=0 To UBound(jobTypes) jobStr=jobStr & jobTypes(i) If i < UBound(jobTypes) Then
    jobStr=jobStr & ", " End If Next ElseIf Len(Trim(jobTypes))> 0 Then
    jobStr = jobTypes ' Only one selected
    Else
    jobStr = "None Selected"
    End If
    ...
    Response.Write "<a href='registration.html'>Go Back</a>"
    %>